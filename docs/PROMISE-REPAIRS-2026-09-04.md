...
The 2026-09-06 replay clarity pass keeps the completed match verdict and final score visible independently from the selected decision receipt. Each step identifies its decision number, paired deal, and first or second seat run. Public viewers receive only outcome counts; an authenticated owner may see aggregate counts of their own actions. This makes real policy behavior legible without publishing opponent actions, policy rules, or invented live timing. A dev.fun Poker Playground review confirmed that its street-by-street spectacle depends on exposing actions and agent reasoning, which is incompatible with Veil Arena's current sealed-strategy promise unless the product adopts delayed disclosure or a separate transparent mode.
...
### 2026-09-06 timed competition and private viewing follow-up

- Version 3 rules schedule complete repeated pairing rounds from the published start through just before the published end. Hosts select a per-agent qualification sample of 100, 500, 1,000, 5,000 or 20,000 decisions; presets choose an appropriate default.
- Migration 0025 adds nullable round and scheduled-time fields. Legacy version 1 and 2 seasons retain their historical ten-second sequence calculation and receipts.
- Due tables run concurrently within a configured limit of 1–8, default 4. The batch selector rotates across competitions before taking another table from the same competition. Lease recovery, stable seeds, retry fencing and the existing maximum attempt count still apply.
- Anonymous and non-entrant viewers receive only completed result summaries and the competition leaderboard. Entrants can open every scheduled table in their competition and replay its receipt. Only a player in the exact match receives that player's post-result cards and recorded actions. Public result summaries omit per-hand receipts and selective reveal data.
- Per-competition standings use the same shared 3/1/0 projection as reward settlement. Equal leaders share the displayed rank; a tied competition is not presented as a winner.
- The Arena notification bell polls owner entry outcomes from any Arena page and announces a completed win or result once per browser session. Match pages also announce their own completed result.
- Final local validation: optimized Next build passed TypeScript, ESLint, 60 test files and 258 active tests; Playwright passed 100/100 desktop/mobile cases; the deterministic transcript proof and worker timeout/restart/health smoke passed; `npm audit --omit=dev` reported zero vulnerabilities.
- Disposable PostgreSQL 16 applied all 26 migrations twice and passed 31/31 repository, draft, duplicate-package and worker-recovery tests plus 10/10 unchanged promise probes. The labelled test-only container `veil-timed-audit-20260906` was verified and removed. No production database was contacted.
- The unchanged 256-match champion benchmark completed with zero failures. Held-out W/L/T remains fold 24/0/0, call 20/4/0, raise 24/0/0, Value Bot 14/4/6, Null Jack v1 12/2/10 and same-policy 0/0/8.
- This follow-up schedules real repeated deterministic matches. It does not implement live poker streets or claim that completed computation is live play. The separate runtime and privacy contract is documented in `docs/COMPETITION-RUNTIME-SPEC-2026-09-06.md`.
...
| F07 live-game gap | Explicitly retained the decision benchmark; status and result-replay labels replace live-decision claims. Match verdict, current decision outcome, paired deal and seat run are now separate. Owners see only their own aggregate action profile. Required incremental architecture and paired-card reveal policy are documented above before any engine expansion. | Actual street-by-street live poker remains outside this repair. No recorded action timing is invented. |
...
- Worker health follow-up: expected automatic-lock blocks (`ARENA_SEASON_TOO_SMALL`, `ARENA_PRIZE_POOL_NOT_FUNDED`, and `ARENA_BENCHMARK_REQUIRED`) remain visible as competition states but no longer mark the worker unhealthy. This prevents abandoned or not-yet-funded open competitions from forcing a release rollback; persistence, authorization and execution failures still fail closed.
...
- Automatic roster lock: join-lock-route.test.ts confirms a failed lock returns an explicit failure with enrollment-saved context. A successful enrollment is not presented as successful automatic scheduling. The worker now locks any due non-gauntlet season with a stable idempotency key and runs its first eligible pairing; gauntlets remain explicit because a benchmark must be selected.
- Sponsor amount UX: the operator enters normal token units with an explicit decimals field (USDC defaults to 6). Exact string arithmetic converts the value to protocol minor units, rejects zero/negative/over-precision input, and shows the readable amount alongside the stored integer.
- Curated reward tokens: the operator now selects canonical Starknet Mainnet USDC or STRK; contract addresses and decimals are fixed in the application, the wallet approves the exact selected amount, and the verified winner payout remains a private transfer to the participant wallet recorded at entry.
- Historical data and unrelated changes: no production data was connected or rewritten. Original audit report, benchmark policy/seed set, old receipt rules, .gitignore and ui-artifacts were preserved. Migrations 0021 and 0022 remain additive promise repairs, and migration 0025 adds only nullable schedule metadata plus a due-work index. Owner rewrap changes ciphertext only when explicitly invoked.
...
- [ ] Before rollout: restore a backup into staging; review and apply every migration missing from production, including additive 0021, 0022 and 0025; verify old agent IDs/versions, receipt signatures/digests and results against the backup. Configure the independent participant vault ring and retained legacy keys. Set `VEILAP_ARENA_WORKER_CONCURRENCY` from 1–8, starting at 4, then verify readiness and fresh worker heartbeats.
...
- [ ] Create a timed two-agent competition with a small qualification sample. Confirm both accounts enter before lock, all rounds appear at lock, the first table is not eligible before its published time, and later rounds execute across the full window instead of finishing immediately. Repeat with several competitions and confirm due tables progress concurrently without one competition monopolizing the worker.
- [ ] As an anonymous visitor and a third signed-in non-entrant, open the competition: only completed results and that competition's leaderboard may be visible. Both entrants may open every queued, running and completed table in their competition. In an exact match, each player sees only their own post-result cards/actions; neither sees the opponent policy or reasoning.
...
- [ ] Leave the winning account on another Arena page when the competition finishes. Confirm the notification bell announces the win once, links to the correct competition and does not disclose private match actions. Confirm tied and non-winning accounts receive accurate completion text.
