...
An operator first selects Challenge a friend, Public freepass, or Sponsored competition. The form then asks only for fields that change that format. The operator names the event, sets its lock time, chooses freepass or a funded reward, enters the exact token amount when funded, reviews the privacy and workload summary, then creates it. There is no seeded reward amount. A funded competition asks for the wallet only after the exact pool is prepared; the operator connects, opens the wallet, submits, and verifies the expected receipt before treating the reward as funded. The system creates the underlying project automatically. Project IDs remain available in technical details but are not setup inputs.
