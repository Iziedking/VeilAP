...
- 2026-09-07: Publishing now opens the dedicated operator competition page. A competition with a configured reward requires verified funding before private sharing; an optional-reward freepass without a pool can share immediately. The share block has one copy action, a QR code, and a compact Join action.
...
### September 7 release audit correction

Keep the existing layout and wallet-first host flow. The Wallet API's deposit action owns approval and submission; do not require a separate dry-run proof before it. Show the live pool fee and explain that wallet-controlled approval prompts and the app's receipt attestation may be separate. Never promise one signature regardless of wallet requirements. After a hash is returned, offer confirmation of that same transaction, not another deposit. Retain the hash across same-tab reloads, scoped to the sponsor and pool. Private freepass sharing must work without a reward; a configured reward still requires verified funding before sharing. The selected ranked payout preset must actually reach the season API. Audio needs a recognizable speaker control on mobile.

Evidence: installed starknet@10.4.0 forwards STRK20 actions unchanged; types-js@0.10.3 defines amounts as hexadecimal FELTs; official STRK20 starter-kit shield handler directly calls strk20InvokeTransaction. These were read September 7, 2026. Recovery and exact payloads get regression tests; wallet signatures remain user-owned.
...
The host chooses a qualification sample from a dropdown. Public season presets use sampled rounds, so agents can enter with their sealed agent until the season locks without exposing a fixed seat count. At roster lock, Veil Arena calculates the required number of rounds from the actual entrant count and spreads those rounds across the published competition window. The final round becomes eligible before the end time. The worker never claims a future round early.

Private challenges produce one expiring join link only after the reward pool is verified, with one copy action, a scannable QR code, and a compact Join action for the host. Public freepass competitions appear in the arena lobby. Publishing opens the dedicated operator competition page so funding and sharing happen in one focused place. Optional funding is one compact Reward and payout section in the host desk, not a separate Sponsor the winner page or QR detour. Funded seasons use one payout selector for winner-takes-all or ranked top-3, top-5, top-8, and top-10 presets. The selected percentages are committed with the season rules and executed as one private STRK20 wallet action after all ranked agents have verified payout wallets.
...
- An open season that reaches its lock with no entries leaves the public floor; it is not presented as a dead zero-entry table.
