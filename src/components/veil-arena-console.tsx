    ...
    <div className="operator-page">
      ...
      <main className="operator-main">
        <section className="operator-intro" aria-labelledby="operator-title">
          ...
          {!fundingAccount ? <small className="operator-wallet-required">Connect your Starknet wallet above first. It signs you in and unlocks the operator actions.</small> : null}
        ...
        <div className="operator-grid" id="operator-controls">
            {!manageMode ? <section className="operator-panel operator-panel-wide" aria-labelledby="season-create-title">
              ...
              <form className="operator-form operator-simple-create" onSubmit={(event) => void createSeason(event)}>
                ...
                <button className="operator-button operator-button-dark" type="submit" disabled={busy !== "" || !fundingAccount}>{busy === "create" ? "PUBLISHING" : fundingAccount ? "PUBLISH COMPETITION" : "SIGN IN TO PUBLISH"}<span>+</span></button>
              </form>
            ...
            {projectId ? <section className="operator-panel" aria-labelledby="strategy-title">
              ...
              <p className="operator-panel-copy">Registering an agent adds its sealed package to this competition&apos;s roster. The operator sees the name and commitment, never the strategy. After the lock time, only registered agents can play.</p>
            ...
            {schedule && (schedule.season.status === "open" || schedule.season.status === "locked") && (schedule.season.rules?.rewardPolicy === "funded_before_start" || prizePool) ? <section className="operator-panel operator-panel-wide" aria-labelledby="pool-title">
              ...
              {prizePool?.status === "funded" && schedule.season.status === "open" ? <div className="operator-chain-step"><div><span>REWARD FUNDED</span><strong>The competition now shows a funded reward</strong><small>Entry stays open until the lock time. Matches start automatically after the deadline.</small></div></div> : null}
